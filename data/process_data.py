...
#we'll load the provided 2 csv files first, then merge them together
    '''
    input 2 csv tables
    output 2 loaded tables within python environment
    '''    
...
# create a dataframe of the 36 individual category columns
    '''
    input 2 loaded tables, we'll perform data cleaning and reshaping here
    output final df
    '''   
...
# currently for each column there are 3 unique values, [0,1,2], we need to convert 2 to 1 so that data becomes binary
    categories_expanded=categories_expanded.replace(2, 1)
...
def save_data(df, database_filepath):
    '''
    input previously built df
    output df saved as .db file in the directory
    '''
